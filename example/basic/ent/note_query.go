...
func (nq *NoteQuery) QueryAuthors() *UserQuery {
...
func (nq *NoteQuery) Clone() *NoteQuery {
	...
	return &NoteQuery{
		...
		withAuthors: nq.withAuthors.Clone(),
...
func (nq *NoteQuery) WithAuthors(opts ...func(*UserQuery)) *NoteQuery {
	query := &UserQuery{config: nq.config}
	for _, opt := range opts {
		opt(query)
	}
	nq.withAuthors = query
	return nq
}
...
func (nq *NoteQuery) sqlAll(ctx context.Context) ([]*Note, error) {
	...
	if query := nq.withAuthors; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Note, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Authors = []*User{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*Note)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(note.AuthorsPrimaryKey[1], fks...))
			},
